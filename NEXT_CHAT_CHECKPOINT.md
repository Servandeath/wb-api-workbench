...
- GUI: API Tester, Keys, Imports, History (плейсхолдеры) — весь core
  (декодер, пинг WB/Ozon, БД-репозитории) готов, но нигде не отрисован;
- key_repository и key_storage/encrypted_file_storage существуют отдельно,
  ещё не сшиты в один "сохранить ключ" flow;
...
- кэширование результата пинга (как в reference-скриптах, по хэшу+дню) —
  не портировано, при реальном GUI-вызове пинг будет дёргаться каждый раз;
...
## Раздел Keys (фундамент готов, GUI — нет)
...
- key_storage.py / encrypted_file_storage.py — уже готовы, хранят
  произвольную строку под именем; для Ozon (два секрета) можно класть
  туда сериализованный JSON, менять их не пришлось.

Не сделано:
1. key_repository и key_storage/encrypted_file_storage ещё не сшиты —
   нет одной функции "сохранить ключ", которая пишет и туда, и туда.
2. Кэш результата пинга (в reference-скриптах — по хэшу токена + дню)
   не портирован. Без него каждый клик по ключу — новый live-запрос.
3. GUI-раздел Keys целиком: вставил токен/credentials -> увидел
   права/статус -> задал имя -> сохранил; таблица со списком ключей и
   статусом; кнопка "проверить" на wb_ping/ozon_ping.

## Следующий шаг

Варианты (не выбрано):
- сшить key_repository + key_storage в одну функцию сохранения ключа;
- кэш результата пинга (data/cache/, уже зарезервирован в config.py);
- GUI-раздел Keys поверх готового фундамента.
